...
export function parseMediaUrl(input) {
  ...
  // 5. NetMirror / Net52 Native Player Embed Links
  if (cleanTarget.includes('net52.cc') || cleanTarget.includes('netmirror') || cleanTarget.includes('nm-cdn') || cleanTarget.includes('makhi4.top')) {
    let playUrl = cleanTarget;
    const hlsMatch = cleanTarget.match(/(?:https?:\/\/[^\/]+)\/hls\/(\d+)\.m3u8(?:\?(.*))?/i);
    if (hlsMatch) {
      const videoId = hlsMatch[1];
      const query = hlsMatch[2] || '';
      playUrl = `https://net52.cc/play.php?id=${videoId}${query ? '&' + query : ''}`;
    }
    const idMatch = playUrl.match(/id=(\d+)/) || playUrl.match(/(\d+)/);
