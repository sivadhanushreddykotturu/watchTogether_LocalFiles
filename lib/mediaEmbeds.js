...
export function parseMediaUrl(input) {
  ...
  if (/\.(m3u8)(?:\?.*)?$/i.test(target) || target.includes('.m3u8') || target.includes('/hls/')) {
    const filename = target.split('/').pop().split('?')[0] || 'HLS Stream';
    let streamUrl = target;
    if (target.includes('net52.cc') || target.includes('makhi4.top') || target.includes('netmirror') || target.includes('nm-cdn')) {
      streamUrl = `/api/proxy/hls?url=${encodeURIComponent(target)}&referer=${encodeURIComponent('https://net52.cc/')}`;
    }
    return {
      type: 'hls',
      url: streamUrl,
