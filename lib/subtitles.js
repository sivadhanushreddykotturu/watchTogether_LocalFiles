...
export async function extractMatroskaMediaTracks(file) {
  ...
  // Traverse EBML to find Info and Tracks
  let pos = 0;
        ...
        if (subId === 0x1549a966) { // Info
          ...
          while (p < subElemEnd) {
            ...
            if (tIdInfo.value === 0xae) { // TrackEntry
              ...
              if (trackType === 17 || (codecId && codecId.startsWith('S_TEXT/'))) {
                subtitleTracks.push({
  ...
  if (subtitleTracks.length > 0) {
    ...
    let buffer = bytes.subarray(pos);
    let currentClusterTimecode = 0;

    async function fillBuffer(minNeeded = 65536) {
      if (fileOffset >= file.size && buffer.length === 0) return false;
      const bytesToRead = Math.min(file.size - fileOffset, CHUNK_SIZE);
      if (bytesToRead <= 0) return buffer.length >= 4;
      const nextChunk = await readFileSlice(file, fileOffset, bytesToRead);
      fileOffset += bytesToRead;
      const combined = new Uint8Array(buffer.length + nextChunk.length);
      combined.set(buffer, 0);
      combined.set(nextChunk, buffer.length);
      buffer = combined;
      return true;
    }

    while (buffer.length > 8 || fileOffset < file.size) {
      if (buffer.length < 65536 && fileOffset < file.size) {
        await fillBuffer();
      }
      if (buffer.length < 4) break;

      const idInfo = readEbmlVint(buffer, 0, false);
      if (!idInfo) {
      ...
      if (buffer.length < idLen + 1) {
        if (fileOffset < file.size) { await fillBuffer(); }
        else break;
      }

      const sizeInfo = readEbmlVint(buffer, idLen, true);
      if (!sizeInfo) {
        buffer = buffer.subarray(1);
        continue;
      }
      ...
      if (id === 0xe7) { // Cluster Timecode
        if (buffer.length < headerLen + elemSize) {
          await fillBuffer(headerLen + elemSize);
        }
        currentClusterTimecode = readEbmlUint(buffer, headerLen, elemSize);
        buffer = buffer.subarray(headerLen + elemSize);
        continue;
      }
      ...
      if (id === 0xa0) { // BlockGroup
        ...
        const bgBytes = buffer.subarray(headerLen, headerLen + elemSize);
        let bgPos = 0;
        let blockData = null;
        ...
        if (blockData && blockData.length >= 4) {
          ...
          if (trkInfo && trackNums.has(trkInfo.value)) {
            ...
            const durMs = blockDuration ? Math.round(blockDuration * timeUnitToMs) : 4000;
      ...
      if (id === 0xa3) { // SimpleBlock
        ...
        const sbBytes = buffer.subarray(headerLen, headerLen + elemSize);
        if (sbBytes.length >= 4) {
          const trkInfo = readEbmlVint(sbBytes, 0, true);
          if (trkInfo && trackNums.has(trkInfo.value)) {
            const track = trackMap.get(trkInfo.value);
            const relTime = (sbBytes[trkInfo.length] << 8) | sbBytes[trkInfo.length + 1];
            const signedRelTime = (relTime & 0x8000) ? (relTime - 0x10000) : relTime;
            const payload = sbBytes.subarray(trkInfo.length + 3);
            const startMs = Math.round((currentClusterTimecode + signedRelTime) * timeUnitToMs);
      ...
      if (elemSize <= buffer.length - headerLen) {
        buffer = buffer.subarray(headerLen + elemSize);
      } else {
        const skipTotal = headerLen + elemSize;
        const skipFromCurrentBuffer = buffer.length;
        const remainingToSkip = skipTotal - skipFromCurrentBuffer;
        fileOffset = Math.min(file.size, fileOffset + remainingToSkip);
