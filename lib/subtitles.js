  ...
  try {
    ...
    if (mkv) {
      ...
      const audio = (mkv.audioTracks || []).map((a, idx) => {
        ...
        const codecClean = a.codec ? a.codec.replace('A_', '').replace('MPEG/L3', 'MP3') : '';
        const codecDisplay = codecClean ? `(${codecClean})` : '';
        const label = `${nameDisplay} ${langDisplay} ${codecDisplay}`.trim();
        return {
          id: `audio-${a.trackNumber || idx + 1}`,
          trackNumber: a.trackNumber || idx + 1,
          index: idx,
    ...
    if (mp4) {
      ...
      const audio = (mp4.audioTracks || []).map((a, idx) => {
        const langDisplay = a.language && a.language !== 'und' ? `[${a.language.toUpperCase()}]` : '';
        const nameDisplay = a.name ? a.name : `Audio ${idx + 1}`;
        const codecClean = a.codec ? a.codec.replace('mp4a', 'AAC') : '';
        const codecDisplay = codecClean ? `(${codecClean})` : '';
        const label = `${nameDisplay} ${langDisplay} ${codecDisplay}`.trim();
        return {
          id: `audio-${idx + 1}`,
