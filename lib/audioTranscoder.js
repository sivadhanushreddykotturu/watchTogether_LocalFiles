...
/**
 * Ultra-fast client-side MKV audio demuxer with leftover streaming and 100% lacing support.
 * Eliminates nested disk seeks and scans large MKVs in seconds.
 */
async function extractRawAudioFromMkv(file, targetTrackNumber = null, onStatus = () => {}) {
  ...
  let curPos = 0;
  const CHUNK_SIZE = 32 * 1024 * 1024; // 32MB read chunks
  let leftover = new Uint8Array(0);

  while (curPos < fileSize) {
    const pct = Math.min(99, Math.round((curPos / fileSize) * 100));
    onStatus(`Extracting audio stream... ${pct}%`);

    const toRead = Math.min(CHUNK_SIZE, fileSize - curPos);
    const slice = file.slice(curPos, curPos + toRead);
    const arrayBuf = await slice.arrayBuffer();
    const rawChunk = new Uint8Array(arrayBuf);
    curPos += toRead;

    let chunk = rawChunk;
    if (leftover.length > 0) {
      const merged = new Uint8Array(leftover.length + rawChunk.length);
      merged.set(leftover, 0);
      merged.set(rawChunk, leftover.length);
      chunk = merged;
    ...
    while (cp < chunk.length - 8) {
      ...
      // If the full element is not inside this chunk, save remainder as leftover!
      if (cp + hLen + elemSize > chunk.length) {
        leftover = chunk.slice(cp);
        break;
      }
...
export async function transcodeAudioToMp3(file, targetTrackNumber = null, onProgress = () => {}, onStatus = () => {}) {
  ...
  const outName = `output_${Date.now()}.m4a`;
  ...
  try {
    if (audioData && audioData.length > 0) {
      onStatus(`Transcoding ${(audioData.length / (1024 * 1024)).toFixed(1)}MB pure audio frames...`);
    ...
    onStatus('Converting EAC-3 to seekable stereo M4A...');
    try {
      await ffmpeg.exec([
        '-i', inName,
        '-vn',
